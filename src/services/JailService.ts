...
export class JailService {
  ...
  static async isPlayerInJail(userId: string): Promise<JailStatus> {
    try {
      ...
      const canAffordBribe =
        user.character.cashOnHand + user.character.bankBalance >= bribeAmount;
   ...
   */
  private static async autoReleaseExpiredJail(
    jailRecordId: string
  ): Promise<void> {
    try {
      const db = DatabaseManager.getClient();
      ...
      await db.jail.update({
        ...
        data: {
          ...
          releaseCooldownUntil: new Date(now.getTime() + 5 * 60 * 1000), // 5 minute cooldown
        },
      });
      ...
      logger.error(
        `Failed to auto-release jail record ${jailRecordId}:`,
        error
      );
    }
  }
  ...
  static async checkReleaseCooldown(userId: string): Promise<CooldownStatus> {
    try {
      ...
      const timeLeftMs =
        recentRelease.releaseCooldownUntil.getTime() - now.getTime();
      const timeLeftMinutes = Math.max(0, Math.ceil(timeLeftMs / (1000 * 60)));
  ...
  static async sendToJail(
    ...
    try {
      ...
      if (cooldownStatus.onCooldown) {
        logger.info(
          `Cannot jail player ${userId} - still on release cooldown for ${cooldownStatus.timeLeftFormatted}`
        );
        throw new Error(
          `Player cannot be jailed yet - release cooldown active for ${cooldownStatus.timeLeftFormatted}`
        );
      }
      ...
      const bribeAmount = this.calculateBribeAmount(
        clampedSeverity,
        playerWealth,
        jailTimeMinutes
      );
      ...
      logger.info(
        `Player ${userId} sent to jail for ${jailTimeMinutes} minutes (crime: ${crime}, severity: ${severity})`
      );
    } catch (error) {
   ...
   */
  static async releaseFromJail(
    userId: string,
    reason: string = "time served"
  ): Promise<void> {
    try {
      const user = await DatabaseManager.getUserForAuth(userId);
      if (!user?.character) {
  ...
  static async processBribe(userId: string): Promise<BribeResult> {
    try {
      ...
      if (!jailStatus.canBribe) {
        return {
          success: false,
          message: `You don't have enough money to bribe your way out! Need ${BotBranding.formatCurrency(
            jailStatus.bribeAmount || 0
          )}.`,
        };
      }
      ...
      if (!paymentResult.success) {
        return {
          ...
          message:
            ...
            "Failed to process bribe payment. Please try again.",
        };
      }
      ...
      return {
        success: true,
        message:
          `🤝 **Bribe successful!** You've paid ${BotBranding.formatCurrency(
            bribeAmount
          )} to get out of jail.\n` +
          ...
          `⏳ You cannot be jailed again for 5 minutes.`,
      ...
      return {
        success: false,
        message:
          "An error occurred while processing your bribe. Please try again.",
      };
    }
  }
   ...
   */
  private static async deductMoneyForBribe(
    userId: string,
    amount: number
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (totalAvailable < amount) {
        return {
          success: false,
          message: `Insufficient funds. You have ${BotBranding.formatCurrency(
            totalAvailable
          )} available (cash + bank), need ${BotBranding.formatCurrency(
            amount
      ...
      await db.bankTransaction.create({
        data: {
          ...
          description: `Jail bribe payment - ${BotBranding.formatCurrency(
            amountFromCash
          )} cash + ${BotBranding.formatCurrency(amountFromBank)} bank`,
        },
      });
      ...
      logger.error(
        `Failed to deduct money for bribe (user: ${userId}, amount: ${amount}):`,
        error
      );
      return {
        success: false,
   ...
   */
  static async checkJailBlocking(
    userId: string,
    actionType: string
  ): Promise<{
    blocked: boolean;
    ...
    if (blockedActions.includes(actionType)) {
      return {
        blocked: true,
        reason:
          `🚔 You're in jail and cannot perform this action!\n` +
          `⏰ Time remaining: **${jailStatus.timeLeftFormatted}**\n` +
          `💰 Bribe cost: **${BotBranding.formatCurrency(
            jailStatus.bribeAmount || 0
          )}**\n` +
