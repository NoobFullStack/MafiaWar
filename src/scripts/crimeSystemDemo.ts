  ...
  try {
    ...
    for (let level = 1; level <= 5; level++) {
      ...
      crimes.forEach(crime => {
        console.log(`  - ${crime.name} (Difficulty: ${crime.difficulty}, Reward: $${crime.rewardMin}-${crime.rewardMax})`);
      });
    }
    ...
    if (testCrime) {
      ...
      const successRate = CrimeService.calculateSuccessRate(testCrime, playerStats, 3);
      console.log(`${testCrime.name} success rate for Level 3 player: ${(successRate * 100).toFixed(1)}%`);
      console.log(`Base rate: ${(testCrime.baseSuccessRate * 100)}%, with stat bonuses applied`);
    }
    ...
    allCrimes.forEach(crime => {
      ...
      if (levelReq > 1) {
        console.log(`${crime.name}: Requires Level ${levelReq} (${crime.category})`);
      }
    });
    ...
    const simulateCrime = (crimeId: string, playerLevel: number) => {
      ...
      const successRate = CrimeService.calculateSuccessRate(crime, playerStats, playerLevel);
      const avgReward = (crime.rewardMin + crime.rewardMax) / 2;
      
      console.log(`${crime.name} (Level ${playerLevel}):`);
      console.log(`  Success Rate: ${(successRate * 100).toFixed(1)}%`);
      console.log(`  Expected Value: $${(avgReward * successRate).toFixed(0)} per attempt`);
