...
import { LevelGateValidator } from "../utils/LevelGateValidator";
...
function demonstrateLevelGating() {
  ...
  testPlayers.forEach((player, index) => {
    console.log(`\n👤 PLAYER ${index + 1} - Level ${player.level} (${player.experience} XP)`);
    console.log("-".repeat(50));

    // Show available items
    const availableItems = LevelGateValidator.getAvailableItems(gameItems, player);
    ...
    // Show locked items
    const lockedItems = gameItems.filter(item => 
    ...
    if (lockedItems.length > 0) {
      ...
      lockedItems.forEach(item => {
        ...
        const message = LevelGateValidator.getUnlockMessage(validation, player.level);
        console.log(`  ${message} ${item.name}`);
      });
    }

    // Show available crimes
    const availableCrimes = LevelGateValidator.getAvailableCrimes(crimeData, player);
    ...
    // Show locked crimes
    const lockedCrimes = crimeData.filter(crime => 
    ...
    if (lockedCrimes.length > 0) {
      ...
      lockedCrimes.forEach(crime => {
        const validation = LevelGateValidator.validateCrimeAccess(crime, player);
        const message = LevelGateValidator.getUnlockMessage(validation, player.level);
    ...
    const availableAssets = LevelGateValidator.getAvailableAssets(assetTemplates, player);
    ...
    // Show locked assets
    const lockedAssets = assetTemplates.filter(asset => 
    ...
    if (lockedAssets.length > 0) {
      ...
      lockedAssets.forEach(asset => {
        const validation = LevelGateValidator.validateAssetAccess(asset, player);
        const message = LevelGateValidator.getUnlockMessage(validation, player.level);
    ...
    const upcomingUnlocks = LevelGateValidator.getUpcomingUnlocks(
      gameItems, crimeData, assetTemplates, player
    );
    
    if (Object.keys(upcomingUnlocks).length > 0) {
      console.log(`\n🎯 UPCOMING UNLOCKS:`);
      Object.entries(upcomingUnlocks).forEach(([level, content]) => {
        const xpNeeded = LevelGateValidator.getXPToUnlock(parseInt(level), player.level);
        console.log(`  📈 Level ${level} (${xpNeeded} XP needed):`);
        
        if (content.items.length > 0) {
          console.log(`    🛠️  Items: ${content.items.map(i => i.name).join(', ')}`);
        }
        if (content.crimes.length > 0) {
          console.log(`    ⚡ Crimes: ${content.crimes.map(c => c.name).join(', ')}`);
        }
        if (content.assets.length > 0) {
          console.log(`    🏢 Assets: ${content.assets.map(a => a.name).join(', ')}`);
        }
      });
    }
  ...
  milestones.forEach(level => {
    ...
    console.log(`\nLevel ${level}: ${milestone?.title || 'No milestone'} (${totalXP} XP total)`);
    
    // Show what unlocks at this level
    const itemsAtLevel = gameItems.filter(item => item.metadata?.levelRequirement === level);
    const crimesAtLevel = crimeData.filter(crime => crime.requirements?.level === level);
    const assetsAtLevel = assetTemplates.filter(asset => asset.requirements?.level === level);
    
    if (itemsAtLevel.length > 0) {
      console.log(`  🛠️  Items: ${itemsAtLevel.map(i => i.name).join(', ')}`);
    }
    if (crimesAtLevel.length > 0) {
      console.log(`  ⚡ Crimes: ${crimesAtLevel.map(c => c.name).join(', ')}`);
    }
    if (assetsAtLevel.length > 0) {
      console.log(`  🏢 Assets: ${assetsAtLevel.map(a => a.name).join(', ')}`);
    }
  });
}
