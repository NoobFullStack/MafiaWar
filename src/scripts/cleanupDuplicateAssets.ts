...
async function cleanupDuplicates(dryRun: boolean = true): Promise<void> {
  try {
    ...
    console.log(`⚠️  Found ${duplicates.length} groups with duplicate assets:\n`);
    ...
    for (const group of duplicates) {
      ...
      group.assets.sort((a, b) => a.lastIncomeTime.getTime() - b.lastIncomeTime.getTime());
      
      console.log("   Assets:");
      group.assets.forEach((asset, index) => {
        const isKeep = index === 0;
        console.log(`   ${isKeep ? '✅ KEEP' : '❌ DELETE'} ${asset.id.substring(0, 8)}... (Level ${asset.level}, Last Income: ${asset.lastIncomeTime.toISOString().split('T')[0]})`);
      });
      ...
      if (!dryRun) {
        ...
        for (const assetToDelete of assetsToDelete) {
          ...
          console.log(`   🗑️  Deleted asset ${assetToDelete.id.substring(0, 8)}...`);
        }
      }
    ...
    if (dryRun) {
      ...
      console.log("💡 To actually delete duplicates, run: yarn cleanup-duplicates --execute");
    } else {
